...
export default function Daily() {
  ...
  const { isSuccess, refetch } = useQuery(
    "daily games",
    () => GameService.getData("daily"),
    {
      skip: true,
      onSuccess: (data) => setdailyGames(data?.data),
    }
  );
  ...
  return (
    <Box
      ...
      position="relative"
    >
      {dailyGames?.map((dailyGamesData, index) => (
        <Box
          key={index}
          w="100%"
          display="flex"
          ...
          <Flex flexDirection="column" w="100%" marginTop="-11vh">
            <Flex justifyContent="end">
              ...
              >
                ...
                {dailyGamesData?.isAlreadyRegistered ? (
                  <Link
                    bgColor="#6BC920"
                    fontSize="xs"
                    color="white"
                    ...
                    borderRadius="20px"
                    px={{ md: "5", lg: "10", xl: "12" }}
                    href={`/gameplay?id=${dailyGamesData?.id}`}
                    py="3"
                    fontWeight="bold"
                  >
                    Enter Battle
                  ...
                  <Link
                    ...
                    py="3"
                    fontWeight="bold"
                    onClick={() => registerToTriviaGame(dailyGamesData?.id)}
                  >
                    Register Now
                  </Link>
                )}
                ...
                <Flex flexDirection="column" gap="1" w="80%">
                  <Flex gap="0">
                    <Flex w="15%" gap="1" color="#716B76" fontSize="xs">
                      {getHours(dailyGamesData?.battleStartDuration?.hours)
                        .split("")
                        .map((digit, index) => (
                          <Text
                            key={index}
                    ...
                    <Flex w="15%" gap="1" color="#716B76" fontSize="xs">
                      {getMinutes(dailyGamesData?.battleStartDuration?.minutes)
                        .split("")
                        .map((digit, index) => (
                          <Text
                            key={index}
                    ...
                    <Flex w="15%" gap="1" color="#716B76" fontSize="xs">
                      {getSeconds(dailyGamesData?.battleStartDuration?.seconds)
                        .split("")
                        .map((digit, index) => (
                          <Text
                            key={index}
              ...
              <Flex w="40%" flexDirection="row" gap="1" paddingRight="5" py="3">
                ...
                <Flex flexDirection="column" gap="1" w="70%">
                  <Flex gap="3">
                    <Flex w="15%" gap="1" color="#716B76" fontSize="xs">
                      {getHours(
                        dailyGamesData?.registrationDeadlineDuration?.hours
                      )
                        .split("")
                        .map((digit, index) => (
                          <Text
                    ...
                    <Flex w="15%" gap="1" color="#716B76" fontSize="xs">
                      {getMinutes(
                        dailyGamesData?.registrationDeadlineDuration?.minutes
                      )
                        .split("")
                        .map((digit, index) => (
                          <Text
                    ...
                    <Flex w="15%" gap="1" color="#716B76" fontSize="xs">
                      {getSeconds(
                        dailyGamesData?.registrationDeadlineDuration?.seconds
                      )
                        .split("")
                        .map((digit, index) => (
                          <Text
